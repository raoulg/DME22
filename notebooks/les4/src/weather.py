...
stations.drop(
    index=[871, 873, 990], inplace=True
)  # Exclude Saba, St. Eustatius and Bonaire
...
def calculate_metric(temps: pd.DataFrame):
    ...
    """
    avg_day_temps = (
        temps.groupby(temps["date_time"].dt.date)
...
def is_user_location_specified():
    ...
    return (
        "user_location" in st.session_state
        and len(st.session_state["user_location"]) > 0
    )
...
selected_station_names = st.multiselect(
    label="Select station(s)", options=station_names
)
...
if is_user_location_specified():  # Put searched location in the center.
    ...
    view_state = pdk.ViewState(
        latitude=user_coordinate[1], longitude=user_coordinate[0], zoom=10
    )
else:
    view_state = pdk.ViewState(  # Use default map center.
        latitude=default_map_center[0], longitude=default_map_center[1], zoom=5.5
    )
...
with map_col:
    ...
    st.pydeck_chart(
        pdk.Deck(
            layers=[selected_stations_layer, all_stations_layer, user_location_layer],
