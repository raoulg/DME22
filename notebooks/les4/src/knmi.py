...
def get_hourly_temperatures(
    days_back: int = 1, stations: list[int] = []
) -> pd.DataFrame:
    ...
    payload = {"start": start, "end": end, "vars": "T", "stns": stns, "fmt": "json"}
    ...
    if resp.status_code == HTTPStatus.OK:
        ...
        if temperatures.shape[0] > 0:
            ...
            temperatures["date_time"] = temperatures[date_col] + pd.to_timedelta(
                temperatures[hour_col], "h"
            )
            temperatures["temperature_degrees_celsius"] = (
                temperatures[raw_temp_col] / 10
            )
...
def get_nearest_n_stations(
    stations: pd.DataFrame, latitude: float, longitude: float, n: int
) -> pd.DataFrame:
...
def avg_temperature_at_location(
    stations: pd.DataFrame,
    temperatures: pd.DataFrame,
    latitude: float,
    longitude: float,
    ...
    avg_temps = temps.groupby("date_time").apply(
        lambda x: np.average(x["temperature_degrees_celsius"], weights=1 / x["dist"])
    )
